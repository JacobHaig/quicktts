...
class Twitch:
    ...
    def stream_connect(self, channel):
        if self.sock:
            self.sock.close()
        self.sock = None
        self.partial = b""
    ...
    def reconnect(self, delay):
        time.sleep(delay)
        self.stream_connect(self.channel)
...
class YouTube:
    ...
    def reconnect(self, delay):
        ...
        time.sleep(delay)
        self.stream_connect(self.channel_id, self.stream_url)

    def stream_connect(self, channel_id, stream_url=None):
        ...
        try:
            iframe_continuation = initial_data["contents"]["twoColumnWatchNextResults"]["conversationBar"]["liveChatRenderer"]["header"]["liveChatHeaderRenderer"]["viewSelector"][
                "sortFilterSubMenuRenderer"
            ]["subMenuItems"][1]["continuation"]["reloadContinuationData"]["continuation"]
        except Exception as e:
